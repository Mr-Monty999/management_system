...
@section('content')
    <div class="row mar-50">
        <form action="{{ route('vehicles.fuels.update', [$vehicle->id, $vehicleFuel->id]) }}" method="post">
            @csrf
            @method('put')
            ...
            <h1>تعديل وقود للآلية {{ $vehicle->name }} رقم {{ $vehicle->number }}</h1>
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">إسم السائق</label>
                <input type="text" value="{{ $vehicleFuel->driver }}" name="driver" class="form-control"
                    id="exampleFormControlInput1" placeholder="إسم السائق">
            </div>
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">إسم المسؤول</label>
                <input type="text" value="{{ $vehicleFuel->responsible }}" name="responsible" class="form-control"
            ...
            <div class="mb-3">
                ...
                <input type="number" value="{{ $vehicleFuel->fuel_quantity }}" name="fuel_quantity" class="form-control"
                    id="exampleFormControlInput1" placeholder="كمية الوقود (لتر)">
            </div>
            <div class="mb-3">
                <label for="exampleFormControlInput3" class="form-label">التاريخ</label>
                <input type="date" value="{{ date('Y-m-d', strtotime($vehicleFuel->date)) }}" name="date"
            ...
            <div class="mb-3">
                ...
                <textarea type="note" name="note" class="form-control" id="exampleFormControlInput1" id="" cols="30"
                    rows="10">{{ $vehicleFuel->note }}</textarea>
            </div>
